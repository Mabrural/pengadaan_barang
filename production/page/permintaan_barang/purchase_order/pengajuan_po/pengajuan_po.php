...
?>
    <div class="x_panel">
      <div class="x_title">
        <h2>Data Pengajuan Purchase Order <small></small></h2>
        
        <form action="laporan/cetak_po.php" method="get">
            <input type="hidden" name="aksi">
            <input type="hidden" name="id_user" value="<?= $id_user;?>">
            <input type="hidden" name="id_no_po" value="<?= $id_no_po;?>">
            <!-- <input type="hidden" name="id_vendor" value="<?= $id_vendor;?>"> -->
            <!-- <input type="hidden" id="select_id" name="select_id" value="<?=$selectedIds?>"> -->
            <a href="?form=tambahPO&id_no_po=<?= $id_no_po?>" class="btn btn-primary btn-sm"><i class="fa fa-plus"></i> Ajukan PO</a>
            ...
            <?php 
            $query = "SELECT * FROM po_barang JOIN vendor ON vendor.id_vendor=po_barang.id_vendor JOIN req_barang ON req_barang.id_req_brg=po_barang.id_req_brg JOIN barang ON barang.kode_brg=req_barang.kode_brg JOIN satuan ON satuan.id_satuan=req_barang.id_satuan WHERE po_barang.id_no_po=$id_no_po";
              // Eksekusi query
              ...
              <tr class="even pointer">
                ...
                <td class="last">
                    ...
                    if ($data['status_req'] == 'Menunggu Persetujuan Dir. HRD') {
                        echo '<a href="?form=ubahPengajuanPO&id_po=' . $data["id_po"] . '" class="btn btn-info btn-sm">Ubah</a> | ';
                        echo '<a href="?form=hapusPengajuanPO&id_po=' . $data["id_po"] . '&id_no_po='.$data["id_no_po"].'" onclick="return confirm(\'Anda yakin ingin menghapus data ini?\')" class="btn btn-danger btn-sm">Hapus</a>';
                    } else {
